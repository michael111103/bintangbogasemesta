...
export default function Navbar() {
 ...
 return (
   ...
   >
         ...
         <div className="hidden md:flex items-center gap-1">
           {navLinks.map((link) => (
             <a
               ...
               className="px-4 py-2 text-sm font-medium text-gray-700 rounded-lg hover:text-[#00B4B4] hover:bg-white/50 transition-all duration-200 cursor-pointer"
             >
               {link.label}
             </a>
           ))}
     ...
     >
       <div className="bg-white/60 backdrop-blur-md border-t border-white/30 px-4 py-3 space-y-1">
         {navLinks.map((link) => (
           <a
             key={link.href}
             href={link.href}
             onClick={(e) => handleNavClick(e, link.href)}
             className="block px-4 py-3 text-sm font-medium text-gray-700 rounded-lg hover:text-[#00B4B4] hover:bg-white/50 transition-all cursor-pointer"
           >
             {link.label}
           </a>
         ))}
